...
можно добавить 3-мерный график - как можно это исследовать -  и показать это потом в виде графиков
густота листвы от расстояяния до реки - умение задать начальные уловия - не обязательно в виде ползунков. освещенность меняется по линейному закону, и потом измеряемые параметры
разные виды графиков
3д и 4д графики - с цветом и формой
эксперимент по изменению параметров

//проблема - выход за границы модели допустимых комбинаций - неплохо бы детектить, обычно связано с резкими изменениями за короткое время, точно разобраться с зацикливанием
//множественные эксперименты
//ограничивать количество аттераций. точности не хватает 


самое главное это графики!
...
3. Реализация ключевых алгоритмов:
   - Обход дерева в глубину для транспорта веществ
   - Визуализация процесса в реальном времени
   - Сохранение и загрузка параметров погоды в текстовый файл
   - Сбор и экспорт статистики (история изменения количества листьев каждого цвета, изменение погоды во времени, время начала и пика листопада)
...
- Экспорт статистики по листьям в CSV для дальнейшего анализа
