#include "weather.hpp"
#include <cmath>     // для floor, sin, cos, fabs
...
void updateRain(std::vector<Precipitation> &raindrops, float rainIntensity, float deltaTime)
{
    ...
    float speed = 300 + rainIntensity * 2;
    float length = 5 + rainIntensity / 100.0f * 15.0f;
...
{
    float N = num_drops * rainIntensity / 100.0f;

    for (size_t i = 0; i < (size_t)N; i++)
    {
        // Капля — это узкий прямоугольник (чёрточка)
        sf::RectangleShape raindrop(sf::Vector2f(1.5f, raindrops[i].length));
        ...
        raindrop.setRotation(sf::degrees(15.0f));
...
void updateSnow(std::vector<Precipitation> &snowflakes, std::vector<Precipitation> &groundSnow, float snowIntensity, float deltaTime)
{
    ...
    float speed = 30 + snowIntensity / 2.0f;
    float length = 1 + snowIntensity / 100.0f * 6.0f;
    ...
    for (auto &drop : snowflakes)
    {
        ...
        if (drop.y > 690)
        {
            ...
            if (groundSnow.size() > 5000)
                groundSnow.erase(groundSnow.begin());
...
void drawSnow(sf::RenderWindow &window, const std::vector<Precipitation> &snowflakes, std::vector<Precipitation> &groundSnow, size_t num_flakes, float snowIntensity)
{
    float N = num_flakes * snowIntensity / 100.0f;

    for (size_t i = 0; i < (size_t)N; i++)
    {

        sf::CircleShape flakes(snowflakes[i].length);
